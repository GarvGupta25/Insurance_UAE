...
export const groups = ['About you', 'Health and cover', 'Funding and preferences'];
type Field = { key: string; label: string; type?: string; options?: string[] };
const registry: Field[][] = [
  [{ key: 'legal_name', label: 'Full name' }, { key: 'date_of_birth', label: 'Date of birth', type: 'date' }, { key: 'nationality', label: 'Nationality' }, { key: 'residency', label: 'Residency', options: ['citizen', 'resident', 'visitor', 'pending'] }, { key: 'emirate', label: 'Emirate', options: ['Dubai', 'Abu Dhabi', 'Sharjah', 'Ajman', 'Fujairah', 'Ras Al Khaimah', 'Umm Al Quwain'] }, { key: 'area', label: 'City / area (optional)' }, { key: 'mobile', label: 'Mobile (optional)' }],
...
export function ProfileEditor({ profile, onSaved }: { profile: any; onSaved: () => void }) {
  ...
  return <section className="profile-editor" aria-label="Editable insurance profile">
    ...
    <div className="form-footer"><small>Only saved details are used in your quotation.</small><button disabled={busy || !Object.keys(draft).length} onClick={save}>{busy ? <LoaderCircle size={16} className="spin"/> : <Check size={16}/>} Save details</button></div>
  </section>;
}
