...
export function ProfileEditor({ profile, onSaved }: { profile: any; onSaved: () => void }) {
  ...
  return <section className="profile-editor" aria-label="Editable insurance profile">
    ...
    {tab === 0 && <label className="upload-area"><UploadCloud size={22}/><span><strong>Autofill from an identity document</strong><small>Optional · JPG, PNG or PDF · 10 MB · up to 5 pages</small></span><input aria-label="Upload identity document" type="file" accept="image/jpeg,image/png,application/pdf" disabled={busy} onChange={e => { if (e.target.files?.[0]) void upload(e.target.files[0]); e.target.value = ''; }}/></label>}
