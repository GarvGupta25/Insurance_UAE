...
export function ProfileEditor({ profile, onSaved, focusMissingRequest = 0 }: { profile: any; onSaved: () => void; focusMissingRequest?: number }) {
  ...
  return <section className="profile-editor" aria-label="Editable insurance profile">
    ...
    <p className="muted">Complete the UAE identity, health and funding details needed for a plan comparison. Optional fields are labelled.</p>
    <div className="profile-tools"><label><input type="checkbox" checked={autoAdvance} onChange={event => setAutoAdvance(event.target.checked)}/> Auto-advance after save</label><small>Step {tab + 1} of {groups.length} · about {groups.length - tab} minute{groups.length - tab === 1 ? '' : 's'} left</small></div>{savedNotice && <p className="saved-notice" role="status">{savedNotice}</p>}<div className="tabs" role="tablist" aria-label="Profile stages">{groups.map((name, i) => <button id={`profile-tab-${i}`} role="tab" aria-selected={tab === i} aria-controls={`profile-stage-${i}`} key={name} onClick={() => setTab(i)}>{i + 1}. {name}</button>)}</div>
    {tab === 0 && <label className="upload-area"><UploadCloud size={22} aria-hidden="true"/><span><strong>Autofill from an identity document</strong><small>Optional · JPG, PNG or PDF · 10 MB · up to 5 pages</small></span><input aria-label="Upload identity document" type="file" accept="image/jpeg,image/png,application/pdf" disabled={busy} onChange={e => { if (e.target.files?.[0]) void upload(e.target.files[0]); e.target.value = ''; }}/></label>}
    ...
    <div id={`profile-stage-${tab}`} role="tabpanel" aria-labelledby={`profile-tab-${tab}`} className="form-grid">{registry[tab].filter(f => {
      if (f.key === 'conditions') return facts.diagnosed_conditions === 'yes';
