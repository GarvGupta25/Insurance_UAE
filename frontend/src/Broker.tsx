...
export function BrokerWorkspace() {
  ...
  return <>
    ...
    <section className="surface"><div className="section-heading"><div><span className="eyebrow">PRIORITIZED WORKLIST</span><h2>What needs attention first</h2></div><span className="badge neutral">{worklist.data?.length || 0} open</span></div>{worklist.isLoading ? <Loading/> : !worklist.data?.length ? <p className="muted">No assigned work is pending.</p> : <div className="broker-list">{worklist.data.map(item => <button className="list-row" key={item.id} onClick={() => setSelectedApplicant(item.applicant_id)}><span className="badge neutral">#{item.priority_rank}</span><div><strong>{item.item_type.replaceAll('_', ' ')}</strong><small>{item.priority_reason}</small></div><span>{item.age_days}d</span></button>)}</div>}{caseDetail.data && <section className="broker-case-detail"><span className="badge neutral">INTERNAL · BROKER ONLY</span><h2>Complete case record</h2><h3>Classification</h3><p>{caseDetail.data.classification.cohort.replaceAll('_', ' ')}</p>{caseDetail.data.classification.flags.map((flag: any) => <p key={flag.code}><strong>{flag.explanation}</strong> {flag.review_relevance}</p>)}<h3>Pending action</h3><p>{caseDetail.data.next_action_needed}</p><h3>Review history</h3>{caseDetail.data.review_history.length ? caseDetail.data.review_history.map((review: any) => <p key={review.decided_at}>{review.action}: {review.note || 'No note'}</p>) : <p>No completed review.</p>}</section>}</section>
