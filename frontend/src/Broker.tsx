...
export function BrokerWorkspace() {
  ...
  return <>
    ...
    <div className="metric-grid broker-metrics"><div><span>Awaiting review</span><strong>{pending.length}</strong><small>Prepared applications needing a decision</small></div><div><span>Needs attention</span><strong>{uncertain.length}</strong><small>Recommendations with tradeoffs or missing terms</small></div><div><span>Reviewed</span><strong>{rows.length - pending.length}</strong><small>Recommendations ready for member confirmation</small></div></div>
    ...
    <button className="quiet broker-refresh" onClick={() => void recommendations.refetch()}><RefreshCw size={16}/> Refresh workspace</button>
  </>;
}
